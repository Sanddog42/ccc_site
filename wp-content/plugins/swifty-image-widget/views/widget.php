...
if(!empty($instance['imgs'])) :
	...
	if($instance['size'] == 'custom'){
		$img_size = 'full';
		$sizes = 'style="width:'.$instance['img_width'].'%; height:'.$instance['img_height'].'%;"';
	} else if($instance['size'] == 'full'){
		$img_size = 'full';
	...
	<?php foreach($imgs as $img) :
		...
		<?php } else { ?>
			<li><img src="<?php echo $imgsrc[0];?>" <?php if($alt != '') { echo 'alt="'.esc_attr($alt).'"'; } ?> <?php echo $sizes; ?>/>
				<?php if($img['caption'] != '') { echo '<span class="sbcaption'.$caption.'">'.$img["caption"].'</span>';} ?>
			</li>
		<?php } ?>
