...
class wps_gfwidget extends WP_Widget {
  ...
  function widget($args, $instance) {
    ...
    if (!empty($instance['pin-icon']) && $instance['pin-icon'] != 'none') {
  ...
  function form($instance) {
    ...
    if (version_compare($phpversion, '5.3.0', '<')) {
      echo '<p>
              To use mailchimp API you need to have at least PHP 5.3.0, your PHP is ' . $phpversion . '
            </p>';
    }
  ...
  static function predefined_backgrounds($selected = '') {
    ...
    foreach (glob(WPS_GFW_PATH . '/assets/images/*.png') as $filename) {
      $filename = str_replace(WPS_GFW_PATH, '', $filename);
      $bg_images[] = WPS_GFW_URL . '' . $filename;
    }
    
    foreach ($bg_images as $index => $img_src) {
      if ($img_src == $selected) {
    ...
    if (class_exists('wps_ectw_bg_addon')) {
      $new_bg_images = wps_ectw_bg_addon::bg_images();
      foreach ($new_bg_images as $key => $image) {
        if ($image == $selected) {
          $output .= '<div class="wps-gfw-bg-sample selected" data-bg-image="' . $image . '" style="background:url(\'' . $image . '\') top left repeat;">&nbsp;</div>';
        } else {
          $output .= '<div class="wps-gfw-bg-sample" data-bg-image="' . $image . '" style="background:url(\'' . $image . '\') top left repeat;">&nbsp;</div>';
        }
      }
    }
